...
class GenericChunkExtractor:
            ...
            if parser and config:
                try:
                    ...
                    for chunk_type, node_types in config.node_types.items():
                        chunks.extend(self._extract_chunks_by_type(
                            tree.root_node, content, file_path, language, chunk_type, node_types
                        ))
                    
                    if chunks:
                        logger.debug(f"Tree-sitter parsing successful for {file_path}: {len(chunks)} chunks")
                        return chunks
                    else:
                        logger.debug(f"Tree-sitter parsing found no chunks for {file_path}, trying fallback")
                        
                except Exception as e:
                    logger.debug(f"Tree-sitter parsing failed for {file_path}: {e}, trying fallback")
        ...
        def traverse(node: Node, parent_name: str = None, parent_type: str = None):
            ...
            if node.type in node_types:
                chunk = self._create_chunk(
                    node, content, lines, file_path, language, 
                    chunk_type, parent_name, parent_type
                )
                if chunk:
                    chunks.append(chunk)
    ...
    def _extract_signature(self, node: Node, content: str, language: str) -> Optional[str]:
            ...
            if node.type in ['function_declaration', 'function_definition', 'method_definition']:
                ...
                for i in range(start_line, min(start_line + 5, len(lines))):
                    ...
                    if language == 'python' and line.endswith(':'):
                        break
                    elif language in ['javascript', 'typescript', 'c', 'cpp', 'java'] and '{' in line:
                        break
    ...
    def _extract_generic_comment(self, node: Node, content: str, config: LanguageConfig) -> Optional[str]:
