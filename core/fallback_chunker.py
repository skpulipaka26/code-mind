...
class FallbackChunker:
    ...
    def chunk_unsupported_file(self, file_path: str, content: str) -> List[CodeChunk]:
        ...
        if self.config.prefer_heuristic:
            ...
            if heuristic_chunks:
                logger.debug(
                    f"Used heuristic chunking for {file_path}: {len(heuristic_chunks)} chunks"
                )
    ...
    def _find_functions(
        self, lines: List[str], file_path: str, content: str
    ) -> List[CodeChunk]:
    ...
    def _find_logical_end(self, lines: List[str], start_idx: int) -> int:
        ...
        for i in range(
            start_idx + 1, min(start_idx + self.config.max_chunk_size, len(lines))
        ):
            line = lines[i].strip()
    ...
    def _fill_gaps_with_content(
        self, chunks: List[CodeChunk], lines: List[str], file_path: str
    ) -> List[CodeChunk]:
