...
class LanguageRegistry:
            ...
            if language_func:
                try:
                    ...
                    if "Incompatible Language version" in str(version_error):
                        logger.debug(
                            f"Version incompatibility for {config.name}: {version_error}"
                        )
                ...
                logger.warning(
                    f"Could not find language function in {config.tree_sitter_module}"
                )
                return False
    ...
    def _create_basic_config(
        self, lang_name: str, module_name: str
    ...
    def is_supported(self, language_or_extension: str) -> bool:
        ...
        return (
            language_or_extension in self.languages
            or language_or_extension in self.extension_map
        )
