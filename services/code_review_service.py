...
class CodeReviewService:
    ...
    async def review_diff(
        ...
        with self.telemetry.trace_operation(
            "review_diff", {"repo_url": repo_url, "context_enabled": context_enabled}
        ):
                ...
                try:
                    ...
                    if context_enabled:
                        ...
                        with self.telemetry.trace_operation("vector_search"):
                            ...
                            search_results = (
                                self.codebase_service.database.search_similar_code(
                                    query_embedding,
                                    repo_url=repo_url,
                        ...
                        if search_results:
                            ...
                            self.logger.info(
                                f"Reranked to {len(reranked_results)} chunks"
                            )
                    ...
                    if context_enabled and reranked_results:
                        ...
                        review_prompt = self.prompt_builder.build_quick_review_prompt(
                            diff_content
                        )
                        review_type = "quick"
    ...
    async def review_pull_request(
        self, pr_diff_url: str, repo_url: str, pr_number: int
    ) -> Optional[ReviewResult]:
        """
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    self.logger.error(
                        f"Failed to download diff: {response.status_code}"
                    )
                    return None
