...
async def create_quick_review(request: QuickReviewRequest, app_request: Request):
    ...
    try:
        ...
        result = await review_service.quick_review(diff_content=request.diff_content)
        ...
        if result:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to generate quick review"
            )

    except Exception as e:
        logger.error(f"Error in quick review: {e}")
        raise HTTPException(status_code=500, detail=f"Error in quick review: {str(e)}")
